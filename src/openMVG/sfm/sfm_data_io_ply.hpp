					...
					for (const auto& view : sfm_data.GetViews())
					{
						...
						if (sfm_data.IsPoseAndIntrinsicDefined(view.second.get()))
						{
							...
							if (isRepeated.count(pose_id) == 1) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
										<< pose.center()(0) << ' '
								...
								else
								{
									...
									stream.write(reinterpret_cast<const char*> (Vec3uc(0, 0, 0).data()), sizeof(Vec3uc));
								}

								continue;
							}

							auto it = sfm_data1.poses.find(pose_id);
							if (it != sfm_data1.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
							...
							it = sfm_data2.poses.find(pose_id);
							if (it != sfm_data2.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
							...
							it = sfm_data3.poses.find(pose_id);
							if (it != sfm_data3.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
							...
							it = sfm_data4.poses.find(pose_id);
							if (it != sfm_data4.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
								...
								{
									stream.write(reinterpret_cast<const char*> (pose.center().data()), sizeof(Vec3));
									stream.write(reinterpret_cast<const char*> (Vec3uc(144, 52, 190).data()), sizeof(Vec3uc));
								}

								continue;
							}

							it = sfm_data5.poses.find(pose_id);
							if (it != sfm_data5.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
								...
								{
									stream.write(reinterpret_cast<const char*> (pose.center().data()), sizeof(Vec3));
									stream.write(reinterpret_cast<const char*> (Vec3uc(158, 251, 162).data()), sizeof(Vec3uc));
								}

								continue;
							}

							it = sfm_data6.poses.find(pose_id);
							if (it != sfm_data6.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
								...
								{
									stream.write(reinterpret_cast<const char*> (pose.center().data()), sizeof(Vec3));
									stream.write(reinterpret_cast<const char*> (Vec3uc(112, 160, 163).data()), sizeof(Vec3uc));
								}

								continue;
							}

							it = sfm_data7.poses.find(pose_id);
							if (it != sfm_data7.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
								...
								{
									stream.write(reinterpret_cast<const char*> (pose.center().data()), sizeof(Vec3));
									stream.write(reinterpret_cast<const char*> (Vec3uc(217, 158, 17).data()), sizeof(Vec3uc));
								}

								continue;
							}

							it = sfm_data8.poses.find(pose_id);
							if (it != sfm_data8.poses.end()) {
								const geometry::Pose3 pose = sfm_data.poses.at(pose_id);
								if (b_write_in_ascii)
								{
									stream
								...
								{
									stream.write(reinterpret_cast<const char*> (pose.center().data()), sizeof(Vec3));
									stream.write(reinterpret_cast<const char*> (Vec3uc(238, 93, 185).data()), sizeof(Vec3uc));
								}
